...
def test_action_mate_false_prevents_reproduction():
    ...
    assert c1.energy == pytest.approx(100.0 - DT * (METABOLISM_RATE_BY_STAGE[LifeStage.ADULT] + IDLE_PENALTY_RATE))
    assert c2.energy == pytest.approx(100.0 - DT * (METABOLISM_RATE_BY_STAGE[LifeStage.ADULT] + IDLE_PENALTY_RATE))
...
def test_juvenile_prevents_reproduction():
    ...
    assert c2.energy == pytest.approx(
        100.0 - DT * (METABOLISM_RATE_BY_STAGE[LifeStage.JUVENILE] + IDLE_PENALTY_RATE)
    )
...
def test_low_energy_prevents_reproduction():
    ...
    assert len(engine.creatures) == count_before
    assert c1.energy == pytest.approx(100.0 - DT * (METABOLISM_RATE_BY_STAGE[LifeStage.ADULT] + IDLE_PENALTY_RATE))
    assert c2.energy == pytest.approx(MIN_ENERGY_TO_MATE - 1.0 - DT * (METABOLISM_RATE_BY_STAGE[LifeStage.ADULT] + IDLE_PENALTY_RATE))
